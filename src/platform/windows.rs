...
/// Install service and wait for completion without exiting the process
pub fn install_service_and_wait() -> bool {
    log::info!("Installing service and waiting for completion...");
    let _installing = crate::platform::InstallingService::new();
    let (_, _, _, exe) = get_install_info();
    let tmp_path = std::env::temp_dir().to_string_lossy().to_string();
    let tray_shortcut = get_tray_shortcut(&exe, &tmp_path).unwrap_or_default();
...
if exist \"{tray_shortcut}\" del /f /q \"{tray_shortcut}\"
    ...
    // 不调用 run_after_run_cmds 和 std::process::exit，让调用者决定怎么处理
    true
}
